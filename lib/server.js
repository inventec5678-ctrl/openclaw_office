...
async function fetchSessions() {
  try {
    ...
    // 轉換為陣列並取得 recent sessions，只包含實際存在檔案的 sessions
    const sessions = Object.values(sessionsData).map((session, index) => {
      ...
      // 過濾掉 session 檔案不存在的項目
      if (!session.sessionFile) return false;
      const fileExists = fs.existsSync(session.sessionFile);
      if (!fileExists) {
        console.log(`Session 檔案不存在，已過濾: ${session.sessionFile}`);
      }
      return fileExists;
    });
    ...
    if (fs.existsSync(sessionsPath)) {
      try {
        ...
        Object.entries(sessionsData).forEach(([sessionKey, session]) => {
          ...
          if (isActive) {
            status = 'running';
            displayTask = taskName || session.now_doing || '執行中...';
          } else if (isRecent) {
            status = 'busy';
            displayTask = taskName || session.now_doing || '忙碌中';
          } else {
