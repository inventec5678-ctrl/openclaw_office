...
async function fetchAgents() {
  try {
    ...
    // 首先讀取 memory/agents/ 目錄下的所有 agent 檔案
    if (fs.existsSync(agentsPath)) {
      ...
      agentFiles.forEach(file => {
        ...
        if (fs.existsSync(sessionsPath)) {
          try {
            ...
            Object.entries(sessionsData).forEach(([sessionKey, session]) => {
              ...
              // 檢查 session key 或 label 是否包含 agent name
              if (sessionKey.toLowerCase().includes(agentKey) || 
                  label.toLowerCase().includes(agentKey)) {
                const age = session.updatedAt ? now - session.updatedAt : null;
                if (age !== null && age < 300000) { // 5分鐘內有活動
                  isActive = true;
                }
                // 追蹤最新的 session
        ...
        else if (rawStatus === 'working' || rawStatus === '執行中' || rawStatus === 'busy') status = 'busy';
        else if (rawStatus === 'online') status = 'online';
        else if (rawStatus === '籌備中' || rawStatus === '籌備' || rawStatus === 'idle') status = 'idle';
        
        // 優先使用 session 的 label 作為 current task
        let currentTask = taskMatch ? taskMatch[1].trim() : '';
        if (latestSession?.label && !currentTask) {
          currentTask = latestSession.label;
        }
    ...
    // 如果沒有從 memory 讀到任何 agent，回退到從 sessions 取得
    if (agentsList.length === 0 && fs.existsSync(sessionsPath)) {
      try {
        const sessionsData = JSON.parse(fs.readFileSync(sessionsPath, 'utf8'));
        
        // 找出 unique agents (包含 subagent)
        const seen = new Set();
        Object.entries(sessionsData).forEach(([sessionKey, session]) => {
          // 解析 key: agent:main:subagent:uuid 或 agent:main:telegram:direct:xxx
          const parts = sessionKey.split(':');
          const isSubagent = sessionKey.includes('subagent');
          
          if (isSubagent && !seen.has(sessionKey)) {
            seen.add(sessionKey);
            
            const label = session.label || `Agent ${parts[parts.length - 1]?.substring(0, 8)}`;
            const age = session.updatedAt ? now - session.updatedAt : null;
            const isActive = age !== null && age < 300000;
            
            agentsList.push({
              id: sessionKey.substring(0, 8).replace(/[^a-z0-9]/gi, ''),
              name: label,
              role: session.subagentRole === 'leaf' ? 'Subagent' : 'Agent',
              status: isActive ? 'running' : 'completed',
              current_task: session.label || '',
              hasMemory: false
            });
          }
        });
